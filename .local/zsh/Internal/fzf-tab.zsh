...
source "$FZF_TAB_HOME"/fzftab/zsh-ls-colors/ls-colors.zsh fzf-tab-lscolors
...
() {
  ...
  if (( ! $fpath[(I)$FZF_TAB_HOME/fzftab] )); then
    fpath+=($FZF_TAB_HOME/fzftab)
  fi

  autoload -Uz is-at-least -- $FZF_TAB_HOME/fzftab/-#ftb*(:t)
