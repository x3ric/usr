...
local desktopx = require("lib.widgets.desktop")
local desktoputils = require("lib.utils.desktop")
local desktop = {}
local wgeometry = require("lib.utils.desktop").wgeometry
...
function desktop:init(args)
	...
	--------------------------------------------------------------------------------
	netspeed.body = desktopx.speedmeter.compact(netspeed.args, netspeed.style)
	ssdspeed.body = desktopx.speedmeter.compact(ssdspeed.args, ssdspeed.style)
	hddspeed.body = desktopx.speedmeter.compact(hddspeed.args, hddspeed.style)
	cpumem.body = desktopx.multimeter(cpumem.args, cpumem.style)
	transm.body = desktopx.multimeter(transm.args, transm.style)
	disks.body  = desktopx.multiline(disks.args, disks.style)
	qemu.body  = desktopx.multiline(qemu.args, qemu.style)
	thermal_chips.body = desktopx.multiline(thermal_chips.args, thermal_chips.style)
	thermal_storage.body = desktopx.multiline(thermal_storage.args, thermal_storage.style)
	fan.body   = desktopx.multiline(fan.args, fan.style)
	vnstat.body = desktopx.multiline(vnstat.args, vnstat.style)
	calendar.body = desktopx.calendar(calendar.args, calendar.style)
	---- Desktop setup
	----------------------------------------------------------------------------------
	local desktop_objects = {
		calendar,  netspeed, hddspeed, ssdspeed, transm, cpumem, disks, qemu, vnstat, fan, thermal_chips, thermal_storage
	}
	if not autohide then
		desktoputils.build.static(desktop_objects)
	else
		desktoputils.build.dynamic(desktop_objects, nil, beautiful.desktopbg, args.buttons)
	end
