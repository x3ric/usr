...
local function default_style()
	...
	return utils.table.merge(style, utils.table.check(beautiful, "desktop.speedmeter.normal") or {})
end
...
local function set_fullchart_info(objects, label, state, style)
	for i, o in ipairs(objects) do
		...
		o.barvalue:set_text(
			label .. style.label.separator .. utils.text.dformat(state[i], style.unit, style.digits, " ")
		)
...
local function fullchart(label_style, progressbar_style, chart_style, barvalue_height, maxm)
	...
	local widg = {}
	chart_style = utils.table.merge(chart_style, { maxm = maxm })
	progressbar_style = utils.table.merge(progressbar_style, { maxm = maxm })
...
function speedmeter.new(args, style)
	...
	local elements_style = {
		label = utils.table.merge(style.label, { draw = "by_edges", color = style.color.gray }),
		progressbar = utils.table.merge(style.progressbar, { autoscale = args.autoscale, color = style.color }),
		chart = utils.table.merge(style.chart, { autoscale = args.autoscale, color = style.color.gray })
	}
