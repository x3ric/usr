...
local dcommon = require("lib.widgets.desktop.common")
...
local function default_style()
	...
	return utils.table.merge(style, utils.table.check(beautiful, "desktop.speedmeter.compact") or {})
end
...
local function value_chart(style, image, maxspeed)
	local chart = dcommon.chart(utils.table.merge(style.chart, { maxm = maxspeed }))
	local progressbar = dcommon.bar.plain(utils.table.merge(style.progressbar, { maxm = maxspeed }))
...
function speedmeter.new(args, style)
	...
	local maxspeed = utils.table.merge(default_maxspeed, args.maxspeed or {})

	style.chart = utils.table.merge(style.chart, { autoscale = args.autoscale, color = style.color.gray })
	style.progressbar = utils.table.merge(style.progressbar, { autoscale = args.autoscale, color = style.color })
	style.label = utils.table.merge(style.label, { draw = "by_edges", color = style.color.gray })
	...
	local function update()
		...
		for i = 1, 2 do
			...
			widg[i].text:set_text(utils.text.dformat(state[i], style.unit, style.digits))
