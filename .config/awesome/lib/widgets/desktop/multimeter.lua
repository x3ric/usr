...
local function default_style()
	...
	return utils.table.merge(style, utils.table.check(beautiful, "desktop.multimeter") or {})
end
...
local function set_info(value, args, upright, lines, icon, last, style)
	...
	for i, line in ipairs(args.lines) do
		...
		lines:set_text(utils.text.dformat(value.lines[i][2], line.unit or style.unit, style.digits), i)
...
function multim.new(args, style)
	...
	--local geometry = utils.table.merge(default_geometry, geometry or {})
	style = utils.table.merge(default_style(), style or {})

	local lines_style = utils.table.merge(style.lines, { progressbar = { color = style.color } })
	local upbar_style = utils.table.merge(style.upbar, { color = style.color })
