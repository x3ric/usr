...
local lines = require("lib.widgets.desktop.common.pack.lines")
...
local function default_style()
	...
	return utils.table.merge(style, utils.table.check(beautiful, "desktop.multiline") or {})
end
...
function dashpack.new(args, style)
	...
	local lines_style = utils.table.merge(style.lines, { color = style.color })
	...
	local function set_raw_state(state, maxm, crit, i)
		...
		if style.lines.show.text or style.lines.show.tooltip then
			local txt = state.off and style.dislabel
			            or utils.text.dformat(state[2] or state[1], style.unit, style.digits)
			pack:set_text(txt, i)
