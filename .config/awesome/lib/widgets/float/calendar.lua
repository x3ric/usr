...
local function default_style()
	local style = {
		...
		fonts                     = { clock           = "Sans 24",
		                              ...
		                              focus           = "Sans 14 Bold",
		                              controls        = "Sans 13" },
		icon                      = { next   = utils.base.placeholder({ txt = "►" }),
		                              prev   = utils.base.placeholder({ txt = "◄" }), },
		clock_format              = "%H:%M",
		date_format               = "%A, %d. %B",
	...
	return utils.table.merge(style, utils.table.check(beautiful, "float.calendar") or {})
end
function calendar:init()
	...
	local function decorate_calendar_cell(widget, flag, date)
		if flag == "month" then
			...
			widget:remove_row(1)
			if widget.forced_num_rows then
