...
local function default_style()
	...
	return utils.table.merge(style, utils.table.check(beautiful, "float.keychain") or {})
end
	...
	self.keygrabber = function(mod, key, event)
		...
		elseif awful.util.table.hasitem(self.service.help,     key) then redtip:show()
			...
			for _, item in ipairs(self.active[3]) do
				if utils.key.match_grabber(item, mod, key) then self:activate(item); return end
			end
		end
	end
end
