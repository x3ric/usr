...
local title = require("lib.widgets.bar.titlebar")
...
local function default_style()
	local style = {
		...
		icon          = {
			title    = utils.base.placeholder({ txt = "[]" }),
			active   = utils.base.placeholder({ txt = "+" }),
			absent   = utils.base.placeholder({ txt = "!" }),
			disabled = utils.base.placeholder({ txt = "X" }),
			hidden   = utils.base.placeholder({ txt = "*" }),
			unknown  = utils.base.placeholder({ txt = "?" }),
		},
		color         = { border = "#575757", text = "#aaaaaa", main = "#b1222b", wibox = "#202020",
		                  gray = "#575757", icon = "#a0a0a0" },
	}

	return utils.table.merge(style, utils.table.check(beautiful, "float.bartip") or {})
end
...
bartip.keys.bar = {
	{
		{ "Mod4" }, "t", function() title.toggle(client.focus); bartip:update() end,
		{ description = "Show/hide titlebar for focused client", group = "Titlebar control" }
	},
	{
		{ "Mod4" }, "a", function() title.toggle_all(); bartip:update() end,
		{ description = "Show/hide titlebar for all clients", group = "Titlebar control" }
	},
	{
		{ "Mod4" , "Shift" }, "t", function() title.global_switch(); bartip:update() end,
...
function bartip:update()
	local name = self.style.names[title._index] or "Unknown"
