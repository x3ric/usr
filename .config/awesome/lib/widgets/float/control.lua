...
local function default_style()
	local style = {
		...
		icon          = {
			resize   = {},
			onscreen = utils.base.placeholder({ txt = "X" }),
		},
		color         = { border = "#575757", text = "#aaaaaa", main = "#b1222b", wibox = "#202020",
		                  gray = "#575757", icon = "#a0a0a0" },
	}

	style.icon.resize[RESIZE_MODE.FULL] = utils.base.placeholder({ txt = "F" })
	style.icon.resize[RESIZE_MODE.HORIZONTAL] = utils.base.placeholder({ txt = "H" })
	style.icon.resize[RESIZE_MODE.VERTICAL] = utils.base.placeholder({ txt = "V" })

	return utils.table.merge(style, utils.table.check(beautiful, "float.control") or {})
end
...
local function control_off_screen(window)
	...
	utils.placement.no_offscreen(window, nil, wa)
end
...
function control:center()
	...
	utils.placement.centered(self.client, nil, mouse.screen.workarea)
...
function control:show()
	...
	if not self.wibox.visible then
		...
		if not is_floating then
			notify:show(utils.table.merge({ text = "No floating window focused" }, self.style.notify))
			return
		end
