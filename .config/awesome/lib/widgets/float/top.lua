...
local function default_style()
	...
	return utils.table.merge(style, utils.table.check(beautiful, "float.top") or {})
end
	...
	function self:update_list()
		...
		for i = 1, number_of_lines do
			toplist.items[i]:set({
				...
				mem = utils.text.dformat(proc[i].mem, style.unit, 2, " "),
				pid = proc[i].pid
			})
...
local function click_outside_minitray(c)
    if not top.wibox then top:init() end
	if top.wibox.visible then
		local mx, my = mouse.coords().x, mouse.coords().y
		local wx, wy = top.wibox:geometry().x, top.wibox:geometry().y
		local ww, wh = top.wibox:geometry().width, top.wibox:geometry().height
...
function top:show(srt)
	...
	if self.wibox.visible then
		...
		tip:set_pack("Top process", self.tip, self.style.keytip.column, self.style.keytip.geometry)
	end
end
