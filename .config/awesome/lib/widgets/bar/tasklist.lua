...
local basetask = gauge.tag
local separator = gauge.separator
local menu = require("lib.widgets.desktop.menu")
local svgbox = gauge.svgbox
...
local function default_style()
	...
	style.winmenu = {
		icon                 = { unknown = utils.base.placeholder(),
		                         tag = utils.base.placeholder({ txt = "■" }),
		                         switch_screen = utils.base.placeholder() },
		micon                = { blank = utils.base.placeholder({ txt = " " }),
		                         check = utils.base.placeholder({ txt = "+" }) },
		layout_icon          = { unknown = utils.base.placeholder() },
	...
	return utils.table.merge(style, utils.table.check(beautiful, "widget.tasklist") or {})
end
...
local function get_state(c_group, style)
	...
	local class = string.gsub((c_group[1].class or "undefined"), "^%l", string.upper)
...
local function tasklist_construct(client_groups, layout, data, buttons, style)
	...
	for i, c_group in ipairs(client_groups) do
		...
		task.widg:buttons(utils.base.buttons(buttons, { group = c_group }))
...
local function construct_tasktip(c_group, layout, data, buttons, style)
	...
	for i, c in ipairs(c_group) do
		...
		if buttons then line.field:buttons(utils.base.buttons(buttons, { group = { c }, gap = gap })) end
...
function redtasklist.winmenu:init(style)
	...
	local switchscreen = function() utils.placement.next_screen(last.client); self.menu:hide() end
