...
local default_button_style = {
	list  = { unknown = utils.base.placeholder({ txt = "X" }) },
	color = default_style.color
}
			...
			if model.hidden then
				if c.floating or awful.layout.get(mouse.screen) == awful.layout.suit.floating then
					if c.class == "kitty" then
						c:relative_move(0,model.size,0,-model.size)
					else
						c:relative_move(0,0,0,-model.size)
					end
				end				
			else
				if c.floating or awful.layout.get(mouse.screen) == awful.layout.suit.floating then
...
function titlebar.mark.base(_, style)
	...
	widg._style = utils.table.merge(default_mark_style, style or {})
...
function titlebar.button.base(icon, style, is_inactive)
	style = utils.table.merge(default_button_style, style or {})
