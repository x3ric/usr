...
local startup = require("lib.utils.startup")
...
local function default_style()
	...
	return utils.table.merge(style, utils.table.check(beautiful, "widget.mail") or {})
end
	...
	local function mail_count(output)
		...
		if c then
			...
			if style.need_notify and (count > 0 or force_notify and checks == #maillist) then
				notify:show(utils.table.merge({ text = count .. " new messages" }, style.notify))
			end
		end
...
function mail.new(style)
	...
	-- Initialize vars
	--------------------------------------------------------------------------------
	style = utils.table.merge(mail.style, style or {})
