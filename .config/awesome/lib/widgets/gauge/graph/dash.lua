...
function dashcontrol.new(style)
	...
	function widg:draw(_, cr, width, height)
		...
		for i = 1, style.bar.num do
			...
			local x = (i - 1) * wstep
			local y = style.aperture and height - ((-i/2*hstep)+((height/2))) or height
			local width = style.bar.width
			local height = style.plain and -height or -(i * hstep)
