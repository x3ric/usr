...
local common = require("lib.layouts.common")
...
local function notify(txt)
	if map.notification then rednotify:show(utils.table.merge({ text = txt }, map.notification_style)) end
end
...
function map.new_group(is_vertical)
	...
	hitimer:connect_signal("timeout",
		function()
			utils.service.navigator.hilight.show(map.data[t].set[map.data[t].active].wa)
			hitimer:stop()
			hitimer = nil
		end
	)
...
function map.set_active(c)
	...
	if pack then
		...
		utils.service.navigator.hilight.show(pack.wa)
...
map.maingrabber = function(mod, key)
	for _, k in ipairs(map.keys.all) do
		if utils.key.match_grabber(k, mod, key) then k[3](); return true end
	end
end
...
-- lib navigator support functions
-----------------------------------------------------------------------------------------------------------------------
