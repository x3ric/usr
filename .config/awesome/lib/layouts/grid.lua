...
local function get_rail(c)
	...
	for _, v in ipairs(cls) do
		local lg = utils.client.fullgeometry(v)
...
function grid.move_to(dir, is_rail, k)
	...
	local g = utils.client.fullgeometry(c)
	k = k or 1
	...
	utils.client.fullgeometry(c, ng)
end
...
function grid.move_handler(c, _, hints)
	...
	if is_diff(hg, g, grid.data.cell) then
		utils.client.fullgeometry(c, fit_cell(hg, grid.data.cell))
	end
end
			 ...
			 for _, v in ipairs(_mouse.buttons) do
				if v then
					...
					if is_diff(ng, cg, grid.data.cell) then
						cg = utils.client.fullgeometry(c, fit_cell(ng, grid.data.cell))
					end
