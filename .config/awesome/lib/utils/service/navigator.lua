...
local rednotify = require("lib.widgets.float.notify")
...
local function default_style()
	...
	return utils.table.merge(style, utils.table.check(beautiful, "service.navigator") or {})
end
...
function navigator.make_paint(c)
	...
	function widg:draw(_, cr, width, height)
		...
		local g = utils.client.fullgeometry(widg._data.client)

		cr:set_source(color(style.color.text))
		utils.cairo.set_font(cr, style.titlefont)
		utils.cairo.textcentre.full(cr, { width/2, height/2 - style.linegap / 2 }, index)
		utils.cairo.set_font(cr, style.font)
		utils.cairo.textcentre.full(cr, { width/2, height/2 + style.linegap / 2 }, g.width .. " x " .. g.height)
	end
	return widg
end
function navigator.make_decor(c)
	...
	object.update =  {
		...
		geometry = function() utils.client.fullgeometry(object.wibox, utils.client.fullgeometry(object.client)) end
	}
	function object:set_client(client_)
		object.client = client_
		object.widget:set_client(client_)
		utils.client.fullgeometry(object.wibox, utils.client.fullgeometry(object.client))
...
function navigator:run()
	...
	if not handler then
		rednotify:show(utils.table.merge({ text = "Layout not supported" }, self.style.notify))
		return
	end
	...
	if tip then
		...
		redtip:set_pack(
			...
			self.style.keytip.base.exit and function() layouts.common.action.exit() end -- fix this?
		)
	end
