...
function system.fs_info(args)
	local fs_info = {}
	args = args or "/"
	local line = utils.read.output("LC_ALL=C df -kP " .. args .. " | tail -1")
...
function system.qemu_image_size(args)
	...
	------------------------------------------------------------
	local line = utils.read.output("LC_ALL=C qemu-img info " .. args)
...
-- Get network speed
	function system.net_speed(interface, storage)
		...
		for line in io.lines("/proc/net/dev") do
			...
			if interface.findall == false then
				...
				if name then
					...
					if not storage[name] then --
						storage[name] = { time = now, recv = 0, send = 0 }
					else
						local interval = now - storage[name].time
						if interval <= 0 then interval = 1 end
						local xdown = (recv - storage[name].recv) / interval
						local xup   = (send - storage[name].send) / interval
						up = up + xup
						down = down + xdown
					end
					storage[name].time = now
					storage[name].recv = recv
					storage[name].send = send
				end	
			end
		end
		return { up, down }
...
function system.battery(batname)
	...
	local files = {
		"present", "status", "charge_now", "charge_full", "current_now"
	}
	for _, v in pairs(files) do
		battery[v] = utils.read.file("/sys/class/power_supply/" .. batname .. "/" .. v)
	end
	...
	if     battery.charge_now then remaining, capacity = battery.charge_now, battery.charge_full / 1.055 -- offset
	...
	if rate ~= nil and rate ~= 0 then
		local timeleft
		if     state == "+" and not percent == 100 then timeleft = (tonumber(capacity) - tonumber(remaining)) / tonumber(rate)
		elseif state == "-" and not percent == 100 then timeleft = tonumber(remaining) / tonumber(rate)
...
function system.lmsensors.get(name)
	if os.time() - system.lmsensors.time > system.lmsensors.delay then
		local output = utils.read.output("sensors")
...
--	local output = utils.read.output("sensors | grep " .. args)
--
...
--
--	if args.main then sensors_store = utils.read.output("sensors | grep Core") end
...
function system.thermal.hddtemp(args)
	...
	local output = utils.read.output("echo | curl --connect-timeout 1 -fsm 3 telnet://127.0.0.1:" .. port)
...
function system.thermal.nvoptimus(setup)
	local nvidia_on = string.find(utils.read.output("cat /proc/acpi/bbswitch"), "ON")
...
function system.thermal.nvsmi()
	local temp = string.match(
		utils.read.output("nvidia-smi --query-gpu=temperature.gpu --format=csv,noheader"), "%d%d"
	)
...
function system.thermal.nvprime()
	local temp = 0
	local nvidia_on = string.find(utils.read.output("prime-select query"), "nvidia")
...
function system.transmission.is_running(args)
	...
	return utils.read.output("pidof -x " .. t_client) ~= ""
end
...
function system.proc_info(cpu_storage)
	...
	local output = utils.read.output("ps -eo pid | tail -n +2")
	...
	for line in string.gmatch(output, "[^\n]+") do
		...
		local stat = utils.read.file("/proc/" .. pid .. "/stat")
