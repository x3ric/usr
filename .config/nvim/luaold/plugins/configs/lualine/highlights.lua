...
if transparent then
  require("notify").setup({
    background_colour = "#00000000", -- Replace with your desired RGB hex value
  })
end
...
M.custom = function(config)
  ...
  if vim.g.colors_name == "habamax" or  vim.g.colors_name == "delek" or  vim.g.colors_name == "desert" or  vim.g.colors_name == "morning" or  vim.g.colors_name == "shine" or  vim.g.colors_name == "slate"  or  vim.g.colors_name == "zellner" then
    vim.api.nvim_set_hl(0, "SLSeparator", {fg = util.get_highlight_value("Normal").background})
  else
    draw(groups) 
  end
