...
function buf_screenpos(row, col, win, buf)
    local top = vim.fn.line('w0', win)
    local filler = utils.filler_above(row, win, buf)
    row = row-top+filler
    return utils.win_screenpos(row+2, col, win)
end
...
function Image:display(row, col, buf, keys)
    ...
    if vim.api.nvim_buf_is_valid(buf) then
        ...
        if row == info.botline then
            local screen_row = buf_screenpos(row, 0, win, buf)
        ...
        row, col = buf_screenpos(row, col, win, buf)
    end
